...
The branching model, local hooks, CI gates, branch-protection rules, agent-collaboration workflow, test-style requirements, and coverage policy that govern how every change to this repository is produced and merged. This capability is the policy layer over the source code; it is not itself executable but it is enforced by `.pre-commit-config.yaml`, `.github/workflows/ci.yml`, and the GitHub branch-protection configuration.
...
The `main` branch SHALL be configured with: require PR, require 1 approving review, require all CI status checks green, require branches up-to-date before merging, disallow force-push, disallow direct push, disallow deletion. The `dev` branch SHALL be configured with: require PR, require all CI status checks green, disallow force-push, disallow direct push, disallow deletion. The `dev` branch SHALL NOT require an approving review. Linear history SHALL NOT be required on `main` (incompatible with the merge-commit promotion strategy).
...
### Requirement: Merge strategies are squash for feat→dev and merge-commit for dev→main
Pull requests from `feat/*` branches into `dev` SHALL be squash-merged so that each feature becomes a single commit on `dev`. Pull requests from `dev` into `main` SHALL be merged with a merge commit so that the history of `main` preserves the boundary between promotion batches and keeps each squashed-feature commit visible. Rebase-merge SHALL NOT be used at either level.

#### Scenario: Feature PR squash-merged to dev
- **GIVEN** a PR `feat/foo` → `dev` is being merged
- **WHEN** the merge is performed
- **THEN** a single commit is added to `dev` whose message is derived from the PR title

#### Scenario: Promotion PR merge-committed to main
- **GIVEN** a PR `dev` → `main` is being merged
- **WHEN** the merge is performed
- **THEN** the resulting commit on `main` is a merge commit with two parents (`main` HEAD and `dev` HEAD), preserving each commit on `dev` as visible in `main`'s history
...
The first change after the dev-lifecycle bootstrap that adds meaningful new tests for a given module SHALL set that module's coverage floor to the post-change measured coverage and SHALL enable the coverage gate for that module from that point forward. The floor for a module SHALL never decrease.
